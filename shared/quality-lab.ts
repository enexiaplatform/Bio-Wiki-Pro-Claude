...
function buildWorkflowDemand(input: QualityLabInput, multiplier = 1): WorkflowDemand[] {
  ...
  return values
    ...
    .map(([key, , units]) => workflow(key, units, insourceFactor, multiplier));
}
...
function buildEquipment(
  ...
  const add = (item: EquipmentRecommendation) => {
...
function consumableOpex(consumables: ConsumableForecast[], future: boolean, high: boolean): number {
  return sum(consumables, (item) => {
    const quantity = future ? item.quantityPerMonthFuture : item.quantityPerMonthNow;
    return quantity * 12 * (high ? item.unitCostHighUsd : item.unitCostLowUsd);
  });
}
...
function scenario(
  ...
  return {
    ...
    annualOpexLowUsd: ceil(staffLow + consumableOpex(consumables, future, false) + annualMaintenanceLow),
    annualOpexHighUsd: ceil(staffHigh + consumableOpex(consumables, future, true) + annualMaintenanceHigh),
  };
}
...
export function compileQualityLabBlueprint(rawInput: QualityLabInput): QualityLabBlueprint {
  ...
  const currentWorkflows = buildWorkflowDemand(input);
  const futureWorkflows = buildWorkflowDemand(input, growthMultiplier);
  const equipment = buildEquipment(input, currentWorkflows, futureWorkflows);
