...
describe("Biopharma product and process quality coverage", () => {
  ...
  it("does not represent QC assays as complete biopharma lifecycle coverage", () => {
    const incomplete = BIOPHARMA_CONTENT_MAP.filter((area) => area.status !== "covered-under-review");
    expect(incomplete).toHaveLength(8);
    expect(incomplete.some((area) => area.id === "formulation-fill-finish-and-stability")).toBe(true);
    expect(incomplete.some((area) => area.id === "comparability-tech-transfer-and-lifecycle")).toBe(true);
  });
  ...
  it("registers dedicated upstream and downstream decision content without claiming full coverage", () => {
    ...
    expect(getBiopharmaCoverage("downstream-purification-and-clearance")?.status).toBe("partial");
  });
  ...
  it("registers formulation and stability decision content without claiming qualified coverage", () => {
    ...
    expect(area?.status).toBe("partial");
  });
