...
export function qualityLabPortfolioScaleFromProductCount(productCount: number | null | undefined): QualityLabPortfolioScale {
  ...
  if (productCount <= 10) return "4-10-products";
  if (productCount <= 25) return "11-25-products";
  return "over-25-products";
}
...
export function formatQualityLabReviewBrief(request: QualityLabReviewRequest): string {
  ...
  if (request.project) {
    ...
    lines.push(
      ...
      `Triage: ${project.inputCompletenessPercent}% model completeness; ${project.blockingOpenCount} controlled-use blockers and ${project.importantOpenCount} important inputs open`,
