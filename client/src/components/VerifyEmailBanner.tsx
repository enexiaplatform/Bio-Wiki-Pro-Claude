...
export function VerifyEmailBanner() {
  ...
  return (
    <div className="rounded-xl border border-amber-400/20 bg-amber-400/5 px-4 py-3 mb-6 flex items-center gap-3">
      <MailWarning className="w-5 h-5 text-amber-400 shrink-0" />
      <div className="flex-1 min-w-0">
        <p className="text-sm font-semibold">Confirm your email</p>
        <p className="text-xs text-muted-foreground">
          We sent a verification link to <span className="text-foreground">{user.email}</span>. Confirm it to secure your account.
        </p>
      </div>
      {sent ? (
        ...
        <button
          onClick={resend}
          disabled={busy}
          className="shrink-0 inline-flex items-center gap-1.5 rounded-lg border border-amber-400/30 px-3 py-1.5 text-xs font-semibold text-amber-300 hover:bg-amber-400/10 transition-colors disabled:opacity-60"
        >
