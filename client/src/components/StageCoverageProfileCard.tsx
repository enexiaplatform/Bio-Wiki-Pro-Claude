      ...
      <div className="grid lg:grid-cols-[1.25fr_0.75fr]">
        <div className="p-5 md:p-6">
          ...
          <div className="mt-4 rounded-xl border border-white/10 bg-black/15 p-4"><p className="text-[9px] font-bold uppercase tracking-wider text-slate-600">Applicability</p><p className="mt-2 text-xs leading-6 text-slate-400">{profile.applicability}</p></div>
           ...
           {profile.decisionPackageIds.length > 0 && <div className="mt-4 rounded-xl border border-violet-300/20 bg-violet-300/[0.05] p-3"><p className="text-[9px] font-bold uppercase tracking-wider text-violet-200">Related decision package</p>{profile.decisionPackageIds.map((packageId) => { const item = getDecisionPackage(packageId); return item ? <Link key={packageId} href={`/evidence/packages/${packageId}`} className="mt-2 block text-xs font-semibold text-slate-300 hover:text-violet-100">{item.title}<ArrowRight className="ml-1 inline h-3 w-3" /></Link> : null; })}</div>}
        </div>
