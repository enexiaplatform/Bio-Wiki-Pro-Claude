...
interface Props {
  ...
  checkingCheckoutAvailability: boolean;
  checkoutAvailable: boolean;
  checkoutAvailabilityError: boolean;
...
export function CareerResults({ profile, entitled, checkingAccess, checkingCheckoutAvailability, checkoutAvailable, checkoutAvailabilityError, checkoutLoading, downloadLoading, error, onBack, onEdit, onCheckout, onDownload, onRouteChange }: Props) {
    ...
    <div className="mx-auto max-w-[1440px] px-4 pb-28 pt-4 md:px-6 md:pt-7">
      ...
      <section className="mt-6 grid gap-4 xl:mt-4 xl:grid-cols-[minmax(0,1.05fr)_minmax(600px,0.95fr)]">
        ...
        <aside className="overflow-hidden rounded-xl border border-amber-300/35 bg-[#071426] p-5 shadow-2xl shadow-black/20 md:p-6 xl:p-4">
          <span className="inline-flex rounded-full border border-amber-300/25 bg-amber-300/[0.06] px-4 py-1.5 text-[10px] font-bold uppercase tracking-[0.18em] text-amber-200">Under review · your personal Blueprint</span>
          <div className="mt-4 grid gap-4 sm:grid-cols-[minmax(220px,0.95fr)_minmax(200px,1.05fr)] sm:items-center xl:mt-2">
            ...
            <div>
              <p className="text-sm font-bold">Personalized Career Operating Blueprint + lifetime execution workspace</p>
          ...
          {entitled ? <div className="mt-5 space-y-2 xl:mt-3"><Link href="/career/blueprint" className="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-teal-400 px-5 py-3.5 text-sm font-bold text-teal-950 transition hover:bg-teal-300 xl:py-3"><FileCheck2 className="h-4 w-4" /> Open my 13-week workspace</Link><button type="button" onClick={onDownload} disabled={downloadLoading} className="inline-flex w-full items-center justify-center gap-2 rounded-lg border border-white/20 px-5 py-3 text-sm font-bold text-slate-100 transition hover:border-white/40 disabled:cursor-wait disabled:opacity-55">{downloadLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Download className="h-4 w-4" />}{downloadLoading ? "Generating your PDF…" : "Download my Career Blueprint"}</button></div> : checkingCheckoutAvailability ? <button type="button" disabled className="mt-5 inline-flex w-full cursor-wait items-center justify-center gap-2 rounded-lg bg-teal-400/60 px-5 py-3.5 text-sm font-bold text-teal-950 xl:mt-3 xl:py-3"><Loader2 className="h-4 w-4 animate-spin" /> Checking secure checkout…</button> : checkoutAvailable ? <button type="button" onClick={onCheckout} disabled={checkingAccess || checkoutLoading} className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-lg bg-teal-400 px-5 py-3.5 text-sm font-bold text-teal-950 transition hover:bg-teal-300 disabled:cursor-wait disabled:opacity-55 xl:mt-3 xl:py-3">{checkingAccess || checkoutLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : <LockKeyhole className="h-4 w-4" />}{checkingAccess ? "Checking access…" : checkoutLoading ? "Opening secure checkout…" : "Unlock my personalized Blueprint — $20 one-time"}</button> : <div className="mt-5 rounded-lg border border-amber-300/20 bg-amber-300/[0.055] p-3 text-left xl:mt-3"><p role="status" className="text-xs leading-5 text-amber-100/80">{checkoutAvailabilityError ? "Checkout availability could not be confirmed." : "Career Blueprint checkout is temporarily unavailable."} Your free Snapshot, proof experiment, and sample remain available.</p><a href="#career-recommendations" className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-lg border border-amber-300/25 bg-amber-300/10 px-4 py-2.5 text-xs font-bold text-amber-100 transition hover:bg-amber-300/15">Continue with my free proof plan <ArrowRight className="h-3.5 w-3.5" /></a></div>}
          {!entitled && <a href="/api/career-blueprint/sample.pdf" download="career-blueprint-illustrative-sample.pdf" onClick={() => analytics.careerSampleDownloaded()} className="mt-2 inline-flex w-full items-center justify-center gap-2 text-xs font-semibold text-teal-300 transition hover:text-teal-200"><Download className="h-3.5 w-3.5" /> Preview a sample Blueprint (PDF)</a>}
          <p className="mt-2 text-center text-[11px] text-slate-400">{entitled ? "Lifetime workspace access · regenerate the PDF whenever your profile changes." : checkoutAvailable ? "Secure checkout · instant PDF · lifetime execution workspace" : "No payment action is shown until checkout availability is confirmed."}</p>
