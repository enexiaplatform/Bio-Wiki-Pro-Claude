...
function ReviewPreview({ input, statuses, interactive, onStatus }: { input: AtlasProMonthlyInput; statuses: Record<AtlasProMonthlyCycleStep["id"], AtlasProMonthlyActionStatus>; interactive: boolean; onStatus?: (id: AtlasProMonthlyCycleStep["id"], status: AtlasProMonthlyActionStatus) => void }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <section className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
        <div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-sky-300">Four-week guided resource path</p><h2 className="mt-2 text-xl font-bold text-white">Use the right depth at the right step</h2></div><p className="max-w-xl text-xs leading-5 text-slate-500">One focused resource for each stage of the monthly cycle. Pro lessons and working files provide the depth; open tools help turn it into action.</p></div>
        <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{review.focus.resources.map((resource, index) => <Link key={resource.label} href={resource.href} className="group rounded-xl border border-white/10 bg-black/15 p-4 transition hover:border-sky-300/25">
          <div className="flex items-center justify-between gap-2"><span className="text-[9px] font-bold uppercase tracking-wider text-slate-500">Week {index + 1} · {resource.cycleStep}</span><span className={`rounded-full border px-2 py-0.5 text-[8px] font-bold uppercase tracking-wider ${resource.access === "pro" ? "border-amber-300/20 bg-amber-300/[0.07] text-amber-200" : "border-white/10 text-slate-500"}`}>{resource.access === "pro" ? "Pro" : "Open"}</span></div>
          <p className="mt-3 text-sm font-bold text-slate-200 group-hover:text-sky-200">{resource.label}</p><p className="mt-1 text-[10px] uppercase tracking-wider text-slate-600">{resource.kind.replaceAll("-", " ")}</p><span className="mt-3 inline-flex items-center gap-1 text-[10px] font-bold text-sky-300">Open resource <ChevronRight className="h-3 w-3" /></span>
        </Link>)}</div>
      </section>
...
export default function AtlasProMonthlyReviewPage() {
  ...
  return <div className="min-h-screen bg-[#07182d] px-4 pb-24 pt-8 text-slate-100"><div className="mx-auto max-w-7xl">
    ...
    <section className="mt-6 rounded-2xl border border-white/10 bg-white/[0.035] p-5 md:p-6" aria-label="Monthly portfolio pulse">
      <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-sky-300">Portfolio pulse</p><h2 className="mt-2 text-xl font-bold text-white">See continuity, not a compliance score</h2><p className="mt-2 max-w-2xl text-xs leading-5 text-slate-500">A longitudinal working view of saved reviews, closed cycles and items needing attention. It does not evaluate site compliance or record quality.</p></div><div className="grid grid-cols-3 gap-2 text-center"><div className="rounded-xl border border-white/10 bg-black/15 px-4 py-3"><p className="text-xl font-bold text-white">{portfolio.uniqueMonths}</p><p className="text-[9px] uppercase tracking-wider text-slate-500">Months</p></div><div className="rounded-xl border border-white/10 bg-black/15 px-4 py-3"><p className="text-xl font-bold text-white">{portfolio.focusCoverage}/5</p><p className="text-[9px] uppercase tracking-wider text-slate-500">Focuses</p></div><div className="rounded-xl border border-white/10 bg-black/15 px-4 py-3"><p className="text-xl font-bold text-white">{portfolio.closedCycles}</p><p className="text-[9px] uppercase tracking-wider text-slate-500">Closed</p></div></div></div>
      {portfolio.items.length === 0 ? <p className="mt-5 rounded-xl border border-dashed border-white/10 p-5 text-xs leading-5 text-slate-500">Your first saved review will start the six-month continuity view and attention queue.</p> : <div className="mt-5 grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
        <div className="rounded-xl border border-white/10 bg-black/15 p-4"><div className="flex items-center justify-between gap-3"><p className="text-xs font-bold text-slate-200">Six-month working completeness</p><span className="text-[9px] uppercase tracking-wider text-slate-600">Input coverage only</span></div><div className="mt-5 flex h-36 items-end gap-3">{portfolio.recentTrend.map((item) => <button type="button" key={item.id} onClick={() => { const record = records.find((candidate) => candidate.id === item.id); if (record) load(record); }} className="group flex h-full min-w-0 flex-1 flex-col items-center justify-end gap-2" title={`Open ${item.focusLabel} review`}><span className="text-[9px] font-bold text-slate-500 group-hover:text-sky-200">{item.readinessPercent}%</span><span className="w-full max-w-12 rounded-t-md bg-gradient-to-t from-sky-500/45 to-sky-300 transition group-hover:to-white" style={{ height: `${Math.max(8, item.readinessPercent)}%` }} /><span className="text-[9px] text-slate-600">{item.month.slice(5)}</span></button>)}</div></div>
        <div className="rounded-xl border border-white/10 bg-black/15 p-4"><div className="flex items-center justify-between gap-3"><p className="text-xs font-bold text-slate-200">Attention queue</p><span className="text-[9px] uppercase tracking-wider text-slate-600">Up to 4</span></div>{portfolio.attentionItems.length === 0 ? <p className="mt-5 text-xs leading-5 text-emerald-200">No open or carried-over review in saved history.</p> : <div className="mt-3 space-y-2">{portfolio.attentionItems.map((item) => <button type="button" key={item.id} onClick={() => { const record = records.find((candidate) => candidate.id === item.id); if (record) load(record); }} className="flex w-full items-center justify-between gap-3 rounded-lg border border-white/10 px-3 py-2.5 text-left transition hover:border-sky-300/25"><div><p className="text-[11px] font-bold text-slate-200">{item.focusLabel} · {readableMonth(item.month)}</p><p className="mt-1 line-clamp-1 text-[10px] text-slate-500">{item.carryover || `Close status: ${statusLabels[item.closeStatus]}`}</p></div>{item.isPastReviewDate && <span className="shrink-0 rounded-full border border-amber-300/20 bg-amber-300/[0.07] px-2 py-1 text-[8px] font-bold uppercase text-amber-200">Past review date</span>}</button>)}</div>}</div>
      </div>}
    </section>
