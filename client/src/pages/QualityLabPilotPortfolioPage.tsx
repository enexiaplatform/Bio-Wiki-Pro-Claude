  ...
  return <div className="min-h-screen bg-[#08111f] px-4 pb-24 pt-8 text-slate-100">
    <div className="mx-auto max-w-6xl">
      ...
      <section aria-labelledby="gate-progress-heading" className="mt-5 rounded-2xl border border-white/10 bg-white/[0.035] p-5 md:p-6">
        ...
        <div className="mt-3 grid grid-cols-2 gap-3 md:grid-cols-5"><div className="rounded-xl border border-white/8 p-3 text-xs text-slate-500"><strong className="block text-lg text-white">{portfolio.deliveryMeasuredCount}</strong>delivery times measured</div><div className="rounded-xl border border-white/8 p-3 text-xs text-slate-500"><strong className="block text-lg text-white">{portfolio.averageDeliveryEffortHours ?? "Open"}</strong>average effort hours</div><div className="rounded-xl border border-white/8 p-3 text-xs text-slate-500"><strong className="block text-lg text-white">{portfolio.averageGrossMarginPercent === null ? "Open" : `${portfolio.averageGrossMarginPercent}%`}</strong>average gross margin</div><div className="rounded-xl border border-white/8 p-3 text-xs text-slate-500"><strong className="block text-lg text-white">{portfolio.totalCorrections}</strong>corrections captured</div><div className="rounded-xl border border-white/8 p-3 text-xs text-slate-500"><strong className="block text-lg text-white">{portfolio.totalDecisions}</strong>buyer decisions captured</div></div>
      </section>
      ...
      {portfolio.records.length === 0 ? <section className="mt-5 rounded-3xl border border-dashed border-white/15 bg-white/[0.025] px-6 py-16 text-center"><h2 className="text-2xl font-bold">No real engagement records yet</h2><p className="mx-auto mt-3 max-w-xl text-sm leading-7 text-slate-400">Submit a Blueprint for expert review, open its engagement workspace and record only evidence that exists. Concept projects and synthetic casebook scenarios do not count.</p><Link href="/quality-lab/projects" className="mt-6 inline-flex items-center gap-2 rounded-xl bg-teal-300 px-5 py-3 text-sm font-bold text-slate-950">Open Blueprint projects <ArrowRight className="h-4 w-4" /></Link></section> : <section className="mt-5 space-y-4">
        {portfolio.records.map((record) => <article key={record.packet.project.id} className="rounded-2xl border border-white/10 bg-white/[0.035] p-5">
          ...
          <div className="mt-4 grid grid-cols-2 gap-3 md:grid-cols-5"><div className="rounded-xl border border-white/8 bg-black/15 p-3 text-xs text-slate-500"><strong className="block text-lg text-white">{record.pilot.deliveryCalendarDays === null ? "Open" : `${record.pilot.deliveryCalendarDays}d`}</strong>delivery time</div><div className="rounded-xl border border-white/8 bg-black/15 p-3 text-xs text-slate-500"><strong className="block text-lg text-white">{record.pilot.deliveryEffortHours ?? "Open"}</strong>effort hours</div><div className="rounded-xl border border-white/8 bg-black/15 p-3 text-xs text-slate-500"><strong className="block text-lg text-white">{record.grossMarginPercent === null ? "Open" : `${record.grossMarginPercent}%`}</strong>gross margin</div><div className="rounded-xl border border-white/8 bg-black/15 p-3 text-xs text-slate-500"><strong className="block text-lg text-white">{record.packet.decisions.length}</strong>decisions</div><div className="rounded-xl border border-white/8 bg-black/15 p-3 text-xs text-slate-500"><strong className="block text-lg text-white">{record.calibration.observedCount}</strong>observed metrics</div></div>
