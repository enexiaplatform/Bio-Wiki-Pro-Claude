...
import { REGULATORY_DOMAIN_SYSTEM_MAP, resolveExplicitSystemStages } from "@/data/resourceConnections";
import { useResourceSelection } from "@/hooks/use-resource-selection";
import { ProjectImpactWatch } from "@/components/quality-lab/ProjectImpactWatch";
import {
  REGULATORY_SOURCES,
...
export default function RegulatoryMonitorPage() {
  ...
  const visibleItems = useMemo(() => {
    if (!monitor) return [];
    const domainFiltered = selectedDomain === "all" ? monitor.items : filterRegulatoryUpdates(monitor.items, { domains: [selectedDomain], sources: [] });
    if (!selection.systemId) return domainFiltered;
    return domainFiltered.filter((item) => item.domains.some((domain) => REGULATORY_DOMAIN_SYSTEM_MAP[domain].some((mapping) => mapping.systemId === selection.systemId && (!selection.stageId || mapping.stageId === selection.stageId))));
  }, [monitor, selectedDomain, selection]);

  useEffect(() => {
    if (loading || loadError || !monitor || !selection.systemId || !selection.stageId || visibleItems.length > 0) return;
    capture("resource_monitor_empty_state_viewed", { system_id: selection.systemId, stage_id: selection.stageId, selected_domain: selectedDomain });
  }, [loadError, loading, monitor, selectedDomain, selection.stageId, selection.systemId, visibleItems.length]);
  ...
  return <div className="min-h-screen bg-[#07182d] px-4 pb-24 pt-8 text-slate-100 md:pt-14"><div className="mx-auto max-w-7xl">
    ...
    {!loading && !loadError && monitor && <ProjectImpactWatch updates={monitor.items.filter((item) => monitor.sources.some((source) => source.id === item.sourceId && source.ok))} />}
    <div className="mt-6"><ResourceSystemNavigator area="monitor" /><StageCoverageProfileCard area="monitor" /></div>
    ...
    <div id="regulatory-feed" className="mt-7 scroll-mt-28"><div className="flex items-center gap-2 overflow-x-auto pb-2">{(["all", ...regulatoryDomainValues] as const).map((domain) => <button type="button" key={domain} onClick={() => { setSelectedDomain(domain); capture("regulatory_monitor_filtered", { domain }); }} className={`shrink-0 rounded-xl border px-3 py-2.5 text-[10px] font-bold uppercase tracking-wider ${selectedDomain === domain ? "border-sky-300/35 bg-sky-300/10 text-sky-200" : "border-white/10 text-slate-500"}`}>{domain === "all" ? "All impact areas" : domainLabels[domain]}</button>)}</div></div>

    {loadError ? (
      <section className="mt-4 rounded-2xl border border-rose-300/20 bg-rose-300/[0.05] p-6"><p className="font-bold text-rose-200">{loadError}</p><p className="mt-2 text-xs leading-6 text-slate-500">No cached feed item is presented as current while the official sources are unavailable.</p></section>
    ) : loading ? (
      <section className="mt-4 rounded-2xl border border-white/10 p-10 text-center text-sm text-slate-500">Refreshing official feeds…</section>
    ) : visibleItems.length === 0 ? (
      <section className="mt-4 rounded-2xl border border-dashed border-amber-300/20 bg-amber-300/[0.04] p-6 text-sm text-slate-400"><strong className="text-amber-200">No mapped signal in the current feed.</strong> This does not prove that the selected system is unaffected; review the full official-source feed and site applicability.</section>
    ) : (
      <section className="mt-4 grid gap-4 lg:grid-cols-2">{visibleItems.map((item) => <article key={item.id} className="flex flex-col rounded-2xl border border-white/10 bg-white/[0.035] p-5"><div className="flex flex-wrap items-center justify-between gap-3"><span className={`rounded-full border px-2.5 py-1 text-[9px] font-bold uppercase ${priorityStyle[item.impactLevel]}`}>{item.impactLevel.replaceAll("-", " ")}</span><span className="text-[10px] text-slate-600">{new Date(item.publishedAt).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" })}</span></div><p className="mt-4 text-[10px] font-bold uppercase tracking-[0.15em] text-sky-300">{item.publisher} · {item.sourceLabel}</p><h2 className="mt-2 text-lg font-bold leading-7">{item.title}</h2><div className="mt-3 flex flex-wrap gap-1.5">{item.domains.map((domain) => <span key={domain} className="rounded-full border border-white/10 px-2 py-1 text-[9px] text-slate-500">{domainLabels[domain]}</span>)}</div><div className="mt-2 flex flex-wrap gap-1.5">{resolveExplicitSystemStages(item.domains.flatMap((domain) => REGULATORY_DOMAIN_SYSTEM_MAP[domain])).map(({ system, stage }) => <span key={`${system.id}:${stage.id}`} className="rounded-full border border-sky-300/15 bg-sky-300/[0.04] px-2 py-1 text-[9px] text-sky-200/70">Possible impact · {system.shortTitle} / {stage.title}</span>)}</div><p className="mt-4 flex-1 text-xs leading-6 text-slate-400">{item.impactNote}</p><div className="mt-4 flex items-center justify-between gap-3 border-t border-white/8 pt-4"><span className="text-[9px] font-bold uppercase tracking-wider text-amber-200/70">{item.documentStatus.replaceAll("-", " ")} · machine triaged</span><a href={item.url} target="_blank" rel="noreferrer" onClick={() => capture("regulatory_source_opened", { item_id: item.id, source_id: item.sourceId, impact_level: item.impactLevel })} className="inline-flex items-center gap-2 text-xs font-bold text-sky-300">Official source <ExternalLink className="h-4 w-4" /></a></div></article>)}</section>
    )}
