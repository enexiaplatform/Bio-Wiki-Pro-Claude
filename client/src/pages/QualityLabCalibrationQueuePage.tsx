  ...
  return <div className="min-h-screen bg-[#08111f] px-4 pb-24 pt-8 text-slate-100">
    <div className="mx-auto max-w-6xl">
      ...
      <header className="mt-6 overflow-hidden rounded-3xl border border-sky-300/20 bg-gradient-to-br from-sky-300/12 via-teal-300/[0.05] to-transparent p-6 md:p-9">
        <div className="flex flex-col gap-6 md:flex-row md:items-start md:justify-between"><div><span className="inline-flex items-center gap-2 rounded-full border border-sky-300/25 bg-sky-300/10 px-3 py-1 text-[10px] font-bold uppercase tracking-[0.18em] text-sky-200"><Network className="h-3.5 w-3.5" /> Atlas Intelligence governance</span><h1 className="mt-5 text-3xl font-bold md:text-5xl">Calibration Learning Review Queue</h1><p className="mt-4 max-w-3xl text-sm leading-7 text-slate-300">Separate project observations from evidence-complete candidates before any benchmark calibration or Domain Pack rule change is considered.</p></div><button disabled={packets.length === 0} onClick={() => downloadLearningCandidateRegistry(packets)} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl bg-teal-300 px-4 py-3 text-sm font-bold text-slate-950 disabled:cursor-not-allowed disabled:opacity-40"><Download className="h-4 w-4" /> Export review registry</button></div>
        <div className="mt-6 flex gap-3 rounded-xl border border-amber-300/15 bg-amber-300/[0.05] p-4 text-xs leading-6 text-slate-400"><ShieldCheck className="mt-0.5 h-5 w-5 shrink-0 text-amber-300" /><p><strong className="text-slate-200">Governance boundary:</strong> eligibility only means the record is ready to enter controlled learning review. It does not approve a rule update, pool confidential customer data or create a benchmark.</p></div>
      ...
      <section className="mt-5 space-y-4">
        {visible.length === 0 ? <div className="rounded-2xl border border-dashed border-white/15 bg-white/[0.025] px-6 py-14 text-center"><h2 className="text-xl font-bold">No calibration records in this view</h2><p className="mt-2 text-sm text-slate-500">Open a Blueprint review workspace to capture actuals, provenance and learning disposition.</p></div> : visible.map(({ packet, candidate, assessment }) => <article key={packet.project.id} className="rounded-2xl border border-white/10 bg-white/[0.035] p-5">
