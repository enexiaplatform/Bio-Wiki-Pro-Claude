...
import {
  assessCasePublicationPermission,
...
export default function QualityLabEngagementPage() {
  ...
  const publicationPermission = assessCasePublicationPermission(packet);
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <section id="pilot-evidence" aria-labelledby="pilot-evidence-heading" className="mt-5 scroll-mt-28 rounded-2xl border border-sky-300/15 bg-sky-300/[0.035] p-5 md:p-6">
          ...
          <div className="mt-5 grid gap-3 md:grid-cols-2 lg:grid-cols-3">
            ...
            <label className="text-xs text-slate-400">Contract value (USD)<input aria-label="Pilot contract value USD" type="number" min="0" step="any" value={packet.pilotControl.contractValueUsd ?? ""} onChange={(event) => updatePilotControl({ contractValueUsd: event.target.value === "" ? null : Number(event.target.value) })} className={`${inputClass} mt-1`} placeholder="Controlled net service value" /></label>
            <label className="text-xs text-slate-400">Direct delivery cost (USD)<input aria-label="Pilot direct delivery cost USD" type="number" min="0" step="any" value={packet.pilotControl.directDeliveryCostUsd ?? ""} onChange={(event) => updatePilotControl({ directDeliveryCostUsd: event.target.value === "" ? null : Number(event.target.value) })} className={`${inputClass} mt-1`} placeholder="Measured direct delivery cost" /></label>
            <label className="text-xs text-slate-400 md:col-span-2 lg:col-span-3">Delivery-economics evidence reference<input aria-label="Pilot delivery economics evidence reference" value={packet.pilotControl.economicsEvidenceReference} onChange={(event) => updatePilotControl({ economicsEvidenceReference: event.target.value })} className={`${inputClass} mt-1`} placeholder="Controlled finance or project-cost record reference; do not paste confidential data" /></label>
        ...
        <WorkspaceDisclosure id="validation-case" title="Controlled validation-case acceptance" description="Promote reviewed project evidence only after baseline, scope, permission, evidence quality and reviewer acceptance are controlled." meta={validationCase.eligibility.replaceAll("-", " ")}>
          <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
            ...
            <label className="text-xs text-slate-400">External publication permission<select aria-label="Validation external publication permission" value={packet.validationControl.publicationPermission} onChange={(event) => updateValidationControl({ publicationPermission: event.target.value as QualityLabEngagementPacket["validationControl"]["publicationPermission"], publicationApprovedAt: ["anonymized-public", "attributed-public"].includes(event.target.value) ? new Date().toISOString() : null })} className={`${inputClass} mt-1`}><option value="not-assessed">Not assessed</option><option value="not-permitted">Not permitted</option><option value="anonymized-public">Anonymized public case permitted</option><option value="attributed-public">Attributed public case permitted</option></select></label>
            ...
            <label className="text-xs text-slate-400">Publication approved by role<input aria-label="Validation publication approved by role" value={packet.validationControl.publicationApprovedByRole} onChange={(event) => updateValidationControl({ publicationApprovedByRole: event.target.value })} className={`${inputClass} mt-1`} placeholder="Client-authorized role" /></label>
            <label className="text-xs text-slate-400 md:col-span-2">Publication permission evidence reference<input aria-label="Validation publication permission evidence reference" value={packet.validationControl.publicationEvidenceReference} onChange={(event) => updateValidationControl({ publicationEvidenceReference: event.target.value })} className={`${inputClass} mt-1`} placeholder="Controlled written permission or contract reference" /></label>
          ...
          <div className="mt-3 rounded-xl border border-sky-300/15 bg-sky-300/[0.04] p-4"><p className="text-[10px] font-bold uppercase tracking-wider text-sky-200">Public case boundary · {publicationPermission.eligibility.replaceAll("-", " ")}</p><p className="mt-2 text-xs leading-5 text-slate-400">Internal learning permission never authorizes external publication. A real case can enter publication review only with explicit scope, evidence reference, approving role and compatible confidentiality classification.</p>{publicationPermission.blockers.length > 0 && <ul className="mt-2 space-y-1 text-xs text-slate-500">{publicationPermission.blockers.map((blocker) => <li key={blocker}>• {blocker}</li>)}</ul>}</div>
        </WorkspaceDisclosure>
