...
export default function MethodStandardsNavigatorPage() {
  ...
  return <div className="min-h-screen bg-[#07182d] px-4 pb-24 pt-8 text-slate-100 md:pt-14"><div className="mx-auto max-w-7xl">
    ...
    <div className="mt-6"><ResourceSystemNavigator area="methods" /></div>

    {selectedSystem && <section className="mt-6"><div className="mb-3"><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-teal-300">Connected method records</p><h2 className="mt-1 text-lg font-bold">{selectedStage ? selectedStage.title : selectedSystem.shortTitle} method context</h2></div>{connectedMethods.length > 0 ? <div className="grid gap-3 md:grid-cols-2">{connectedMethods.map((record) => <article key={record.id} className="rounded-xl border border-teal-300/20 bg-teal-300/[0.05] p-4"><div className="flex items-center justify-between gap-3"><span className={`rounded-full border px-2 py-1 text-[8px] font-bold uppercase ${coverageStyles[record.coverage]}`}>{coverageLabels[record.coverage]}</span><span className="text-[9px] font-bold uppercase text-slate-600">Human review required</span></div><h3 className="mt-3 text-sm font-bold">{record.title}</h3><p className="mt-2 text-xs leading-5 text-slate-500">{record.decision}</p><Link href={hrefWithSelection(record.guideHref, "system-recommendation")} className="mt-3 inline-flex items-center gap-1.5 text-xs font-bold text-teal-300">Open application guide <ArrowRight className="h-3.5 w-3.5" /></Link></article>)}</div> : <div className="rounded-xl border border-dashed border-amber-300/20 bg-amber-300/[0.04] p-5 text-sm text-slate-400"><strong className="text-amber-200">Coverage gap:</strong> no bounded method record is explicitly mapped to this stage.</div>}</section>}
