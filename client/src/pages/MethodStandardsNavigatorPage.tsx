...
export default function MethodStandardsNavigatorPage() {
  ...
  return <div className="min-h-screen bg-[#07182d] px-4 pb-24 pt-8 text-slate-100 md:pt-14"><div className="mx-auto max-w-7xl">
    ...
    <section className="sticky top-16 z-20 mt-6 grid gap-3 rounded-2xl border border-white/10 bg-[#081628]/95 p-3 shadow-2xl shadow-black/20 backdrop-blur md:grid-cols-[1fr_auto]">
      ...
      <div className="flex items-center gap-2 overflow-x-auto"><Filter className="hidden h-4 w-4 text-slate-500 sm:block" />{(["all", "structured-concept", "source-mapped", "evidence-required"] as const).map((value) => <button key={value} type="button" onClick={() => setCoverage(value)} className={`shrink-0 rounded-xl border px-3 py-3 text-[10px] font-bold uppercase tracking-wider ${coverage === value ? "border-teal-300/35 bg-teal-300/10 text-teal-200" : "border-white/10 text-slate-500"}`}>{value === "all" ? "All coverage" : coverageLabels[value]}</button>)}</div>
    </section>
    ...
    {noResults ? <section className="mt-6 rounded-3xl border border-dashed border-amber-300/25 bg-amber-300/[0.04] p-8 text-center"><CircleAlert className="mx-auto h-8 w-8 text-amber-200" /><h2 className="mt-4 text-2xl font-bold">Not yet covered</h2><p className="mx-auto mt-2 max-w-2xl text-sm leading-7 text-slate-400">Atlas will not invent a method or silently infer applicability. Record this as a coverage request so it can be assessed against qualified demand, source access, SME ownership and validation evidence.</p><Link href={reviewHref} onClick={() => capture("method_coverage_requested", { query: query.trim(), result_count: 0 })} className="mt-5 inline-flex min-h-11 items-center gap-2 rounded-xl bg-amber-300 px-5 py-3 text-sm font-bold text-slate-950">Request scoped review <ArrowRight className="h-4 w-4" /></Link></section> : <>
      <section className="mt-8"><div className="flex items-end justify-between gap-4"><div><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-teal-300">Application records</p><h2 className="mt-2 text-2xl font-bold">Methods and decisions</h2></div><span className="text-xs text-slate-500">{result.methods.length} result{result.methods.length === 1 ? "" : "s"}</span></div>
        <div className="mt-4 grid gap-4 lg:grid-cols-2">{result.methods.map((record) => <article key={record.id} className="flex flex-col rounded-2xl border border-white/10 bg-white/[0.035] p-5 md:p-6"><div className="flex flex-wrap items-center justify-between gap-3"><span className={`rounded-full border px-3 py-1 text-[9px] font-bold uppercase tracking-wider ${coverageStyles[record.coverage]}`}>{coverageLabels[record.coverage]}</span><span className="text-[10px] font-bold uppercase tracking-wider text-slate-600">{record.unresolvedCount} unresolved dimensions</span></div><h3 className="mt-4 text-xl font-bold">{record.title}</h3><p className="mt-1 text-[10px] font-bold uppercase tracking-[0.14em] text-sky-300">{record.domain}</p><p className="mt-4 text-sm leading-7 text-slate-300">{record.decision}</p><div className="mt-4 rounded-xl border border-amber-300/10 bg-amber-300/[0.035] p-3 text-xs leading-6 text-slate-500">{record.boundary}</div><div className="mt-4 flex flex-wrap gap-2">{standardsForMethod(record).map((standard) => <a key={standard.id} href={standard.locator} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 rounded-full border border-white/10 bg-black/15 px-2.5 py-1.5 text-[10px] font-semibold text-slate-400 hover:text-white">{standard.title}<ExternalLink className="h-3 w-3" /></a>)}</div><div className="mt-auto flex flex-wrap gap-4 pt-5"><Link href={record.guideHref} className="inline-flex items-center gap-2 text-xs font-bold text-teal-300">Open application guide <ArrowRight className="h-4 w-4" /></Link><Link href="/quality-lab/planner?source=method-navigator" onClick={() => capture("method_navigator_blueprint_opened", { method_id: record.id, coverage: record.coverage })} className="inline-flex items-center gap-2 text-xs font-bold text-sky-300">Use in Blueprint <ArrowRight className="h-4 w-4" /></Link></div></article>)}</div>
      </section>
