...
export default function QualityLabValidationCasesPage() {
  ...
  const calibrationCases = listCalibrationReviewCases();
    ...
    <div className="mx-auto max-w-7xl">
      ...
      <div className="mt-6">
        <QualityLabEditorialHero
          ...
          description="Freeze the estimate-to-actual observation, accept that immutable evidence through controlled review, then confirm scope alignment, learning permission and qualified validation-case acceptance before it counts toward Domain Pack validation."
      ...
      <section className="mt-8 rounded-3xl border border-violet-300/20 bg-violet-300/[0.045] p-5 md:p-7">
        <div className="flex flex-col gap-5 border-b border-white/10 pb-6 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-violet-200">Registry status</p>
            <h2 className="mt-3 text-2xl font-bold">{registry.eligibleCount}/{registry.targetCount} accepted validation cases</h2>
            <p className="mt-2 max-w-3xl text-sm leading-7 text-slate-400">Only distinct projects with accepted immutable calibration evidence, aligned scope, controlled learning permission and qualified case acceptance are counted.</p>
          </div>
          <button type="button" onClick={() => downloadValidationCaseRegistry(packets, calibrationCases)} className="inline-flex w-fit shrink-0 items-center gap-2 rounded-xl border border-violet-300/25 bg-violet-300/10 px-4 py-3 text-xs font-bold text-violet-100 hover:bg-violet-300/15"><Download className="h-4 w-4" /> Export validation registry</button>
        </div>
      ...
      <section className="mt-6 grid gap-3 md:grid-cols-3" aria-label="Evidence record distinctions">
        ...
        <Stage icon={GitCompareArrows} title="Accepted calibration evidence" text="Has an immutable estimate-to-actual observation plus an append-only evidence-review decision. It still cannot validate a rule by itself." tone="text-sky-300" />
        <Stage icon={CheckCircle2} title="Accepted validation case" text="Adds aligned scope, qualification evidence, learning permission and a separate qualified case acceptance to the frozen observation." tone="text-teal-300" />
      </section>
      ...
      <section className="mt-6 space-y-4">
        {registry.records.length === 0 ? (
          <div className="rounded-3xl border border-dashed border-white/15 bg-white/[0.025] px-6 py-14 text-center">
            ...
            <h2 className="mt-4 text-xl font-bold">No validation-case acceptance has started</h2>
            <p className="mx-auto mt-2 max-w-2xl text-sm leading-7 text-slate-500">Create or open a real reviewed project, record qualified actuals, freeze the observation, accept it in the calibration evidence queue, then complete the separate validation-case control.</p>
            <div className="mt-5 flex flex-col justify-center gap-3 sm:flex-row"><Link href="/quality-lab/projects" className="inline-flex items-center justify-center gap-2 rounded-xl bg-teal-300 px-4 py-3 text-sm font-bold text-slate-950">Open project workspace <ArrowRight className="h-4 w-4" /></Link><Link href="/quality-lab/calibration" className="inline-flex items-center justify-center rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm font-bold text-slate-200">Open calibration review</Link></div>
          </div>
        ) : registry.records.map(({ packet, assessment }) => (
          <article key={packet.project.id} className="rounded-2xl border border-white/10 bg-white/[0.035] p-5">
            <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
              <div>
                <span className={`rounded-full border px-2.5 py-1 text-[9px] font-bold uppercase ${eligibilityStyle[assessment.eligibility]}`}>{assessment.eligibility.replaceAll("-", " ")}</span>
                <h2 className="mt-3 text-xl font-bold">{packet.project.name}</h2>
                <p className="mt-1 text-xs text-slate-500">{packet.validationControl.caseId || "Case ID open"} · {packet.sourceVersions.domainPack}</p>
                {assessment.calibrationObservationId && <p className="mt-1 break-all text-[10px] text-sky-300">Accepted observation: {assessment.calibrationObservationId}</p>}
              </div>
              <Link href={`/quality-lab/engagements/${packet.project.id}#validation-case`} className="inline-flex items-center gap-2 text-sm font-bold text-teal-300">Open case control <ArrowRight className="h-4 w-4" /></Link>
            </div>
            <div className="mt-4 grid gap-3 md:grid-cols-3"><Metric value={assessment.observedMetricCount} label="Frozen observed metrics" tone="text-sky-200" /><Metric value={assessment.applicableRuleIds.length} label="Frozen linked rules" tone="text-violet-200" /><Metric value={assessment.blockers.length} label="Case blockers" tone="text-amber-200" /></div>
            {assessment.blockers.length > 0 && <div className="mt-4 rounded-xl border border-amber-300/15 bg-amber-300/[0.04] p-4"><p className="text-[10px] font-bold uppercase tracking-wider text-amber-200">Next evidence required</p><ul className="mt-2 space-y-1 text-xs leading-5 text-slate-400">{assessment.blockers.map((blocker) => <li key={blocker}>• {blocker}</li>)}</ul></div>}
          </article>
        ))}
      </section>
