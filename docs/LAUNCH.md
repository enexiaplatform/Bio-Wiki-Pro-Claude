...
Checklist for going live on a custom domain (e.g. `biowikipro.com`) with
...
### In Vercel
1. Project → **Settings → Domains** → add `biowikipro.com` and `www.biowikipro.com`.
...
| Where | Change |
|---|---|
| **Vercel env `VITE_SITE_URL`** | `https://biowikipro.com` → drives client canonical/og/hreflang/JSON-LD (`client/src/lib/site.ts`). **Build-time → redeploy.** |
| **Vercel env `BASE_URL`** | `https://biowikipro.com` → server Stripe success/cancel URLs, email links, RSS, content endpoints. Runtime. |
| `client/public/sitemap.xml` | Replace all `https://bio-wiki-pro-claude.vercel.app` with the new origin (static file — hand edit). |
...
1. Resend dashboard → **Domains → Add Domain** → `biowikipro.com` (or a subdomain like `mail.biowikipro.com`).
...
| TXT | `_dmarc` | `v=DMARC1; p=none; rua=mailto:dmarc@biowikipro.com` | **DMARC** — policy + reports (start `p=none`, tighten to `quarantine`/`reject` later) |
...
| Where | Change |
|---|---|
| **Vercel env `EMAIL_FROM`** | `BioWikiPro <no-reply@biowikipro.com>` (must be on the verified domain). |
