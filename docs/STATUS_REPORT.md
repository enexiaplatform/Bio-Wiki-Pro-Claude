...
- [x] **Accessibility contrast and regression gate resolved (2026-08-14).** The primary foreground token is dark-on-teal, muted dark-surface text was corrected, and `e2e/accessibility.spec.ts` now enforces thirteen automated WCAG 2.1 A/AA, keyboard-state and 320 CSS-pixel reflow checks across strategic desktop/mobile routes. Human assistive-technology review is still required before any formal conformance claim.
