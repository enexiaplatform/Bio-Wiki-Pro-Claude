...
- **Build-time loader:** `client/src/lib/content.ts` (`listContent`/`getContentBySlug`) reads metadata + reading time from a generated manifest (`client/src/data/content-manifest.json`, built by `script/gen-content-manifest.ts` — runs in `npm run dev` and the build, or `npm run gen:content`). Only **free blog bodies** are eager-imported (`import.meta.glob` over `content/blog/**`) for client rendering; academy/toolkit bodies are **never bundled** (server-gated via `GET /api/content/...` → `GatedContent`), so Pro/paid prose never ships to guests. Adding/editing a file: the manifest regenerates on the next dev start / build.
